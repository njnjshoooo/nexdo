            ...
            {navItems.map((item) => (
              <div key={item.id} className="relative group">
                {item.children?.length ? (
                  ...
                  <Link to={item.url} target={item.openInNewWindow ? '_blank' : undefined} onClick={() => setIsServicesOpen(false)} className={`font-medium text-[14px] leading-[20px] lg:text-[16px] lg:leading-[24px] ${isServicesOpen ? 'text-white' : 'text-stone-600'}`}>{item.label}</Link>
                )}
              </div>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-[12px] lg:gap-[20px]">
            ...
            {isAuthenticated ? (
              <div className="relative" ref={userMenuRef}>
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className={`flex items-center gap-[6px] px-[12px] lg:px-[16px] py-[8px] rounded-full font-medium text-[16px] leading-[24px] ${isServicesOpen ? 'bg-[#F5F0EB] text-[#4A5D3B]' : 'bg-primary text-white'}`}>
                  <UserIcon size={18} />
                  <span>{user?.name}</span>
                ...
                <AnimatePresence>
                  {isUserMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg py-2 border border-stone-100">
                      <div className="px-[16px] py-2 border-b border-stone-100 mb-2">
                        <p className="text-[14px] leading-[20px] font-medium text-stone-800 truncate">{user?.name}</p>
                        <p className="text-[12px] leading-[16px] text-stone-500 truncate">{user?.email}</p>
                      </div>
                      ...
                      <button onClick={() => { logout(); setIsUserMenuOpen(false); navigate('/'); }} className="w-full flex items-center gap-[8px] px-[16px] py-2 text-[14px] leading-[20px] text-red-600 hover:bg-red-50"><LogOut size={16} />登出</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
      ...
      <AnimatePresence>
        {isServicesOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="hidden md:block bg-[#4A5D3B] text-white overflow-hidden border-t border-white/10">
            <div className="max-w-7xl mx-auto px-[16px] sm:px-[24px] lg:px-[32px] pt-8 pb-12">
              <div className="max-w-5xl mx-auto flex justify-center">
                <div className="w-full grid grid-cols-3 gap-y-10 md:grid-cols-6 gap-x-6 text-left">
                  {serviceCategories.map((category) => (
                    <div key={category.id} className="space-y-4">
                      {category.url && category.url !== '#' ? (
                        <Link to={category.url} target={category.openInNewWindow ? '_blank' : undefined} className="text-[18px] leading-[28px] font-bold text-white hover:text-white/80 block mb-6" onClick={() => setIsServicesOpen(false)}>{category.label}</Link>
                      ) : (
                        <h3 className="text-[18px] leading-[28px] font-bold text-white mb-6">{category.label}</h3>
                      )}
                      {category.children?.length > 0 && (
                        <ul className="space-y-3 flex flex-col items-start pt-2">
                          {category.children.map((item) => (
                            <li key={item.id}>
                              <Link to={item.url} target={item.openInNewWindow ? '_blank' : undefined} className="text-[14px] leading-[20px] font-medium text-white/80 hover:text-white block" onClick={() => setIsServicesOpen(false)}>{item.label}</Link>
                            </li>
                          ))}
                        </ul>
                      )}
              ...
              <AnimatePresence mode="wait">
                {!activeSubMenu ? (
                  <motion.div key="main-menu" initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -100, opacity: 0 }} transition={{ type: 'tween', duration: 0.3 }} className="px-[16px] pt-2 pb-6 space-y-1">
                    {navItems.map((item) => (
                      <div key={`nav-item-${item.id}`}>
                        {item.children?.length ? (
                          <button type="button" onClick={() => setActiveSubMenu(item.id)} className="w-full flex items-center justify-between px-3 py-4 text-[18px] leading-[28px] font-bold text-stone-700 hover:bg-stone-50 rounded-xl">
                            <span>{item.label}</span>
                            <ChevronDown size={20} className="-rotate-90 text-stone-400" />
                          </button>
                        ) : (
                          <Link to={item.url} target={item.openInNewWindow ? '_blank' : undefined} className="block px-3 py-4 text-[18px] leading-[28px] font-bold text-stone-700 hover:text-primary hover:bg-stone-50 rounded-xl" onClick={() => setIsMenuOpen(false)}>{item.label}</Link>
                        )}
                      </div>
                    ))}
                    ...
                    <div className="pt-4 border-t border-stone-100 mt-4 px-3 mb-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[14px] leading-[20px] font-bold text-stone-600">字體大小</span>
                        <FontSizeControl />
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-stone-100 mt-2">
                      {isAuthenticated ? (
                        <div className="space-y-2">
                          <div className="px-[16px] py-3 bg-stone-50 rounded-xl mb-2">
                          ...
                          {showAdminLink && (
                            <Link to="/admin" className="w-full flex items-center gap-[12px] px-[16px] py-4 text-stone-700 hover:bg-stone-50 rounded-xl font-bold" onClick={() => setIsMenuOpen(false)}><Shield size={20} className="text-primary" />進入後台</Link>
                          )}
                          
                          <Link to="/profile/settings" className="w-full flex items-center gap-[12px] px-[16px] py-4 text-stone-700 hover:bg-stone-50 rounded-xl font-bold" onClick={() => setIsMenuOpen(false)}><Settings size={20} className="text-stone-400" />個人設定</Link>
                  ...
                  <motion.div key="sub-menu" initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} transition={{ type: 'tween', duration: 0.3 }} className="flex flex-col h-full">
                    <div className="px-[16px] py-4 border-b border-stone-100 sticky top-0 bg-white z-10 flex items-center">
                      <button type="button" onClick={() => setActiveSubMenu(null)} className="flex items-center gap-[8px] text-primary font-bold py-2 px-1"><ChevronDown size={20} className="rotate-90" />返回主選單</button>
                      {(() => {
                        const parent = navItems.find(i => i.id === activeSubMenu);
                        return parent?.url && parent.url !== '#' 
                          ? <Link to={parent.url} className="ml-4 text-[18px] leading-[28px] font-black text-stone-800" onClick={() => { setIsMenuOpen(false); setActiveSubMenu(null); }}>{parent.label}</Link>
                          : <h2 className="ml-4 text-[18px] leading-[28px] font-black text-stone-800">{parent?.label}</h2>;
                      })()}
                    </div>

                    <div className="flex-1 overflow-y-auto max-h-[70vh] px-[16px] py-6 space-y-8">
                      {navItems.find(i => i.id === activeSubMenu)?.children?.map((cat) => (
                        <div key={`nav-child-${cat.id}`} className="space-y-4">
                          <div className="flex items-center gap-[12px]">
                            <div className="w-1 h-6 bg-primary rounded-full" />
                            {cat.url && cat.url !== '#' 
                              ? <Link to={cat.url} target={cat.openInNewWindow ? '_blank' : undefined} className="text-[16px] leading-[24px] font-black text-stone-900" onClick={() => { setIsMenuOpen(false); setActiveSubMenu(null); }}>{cat.subtitle ? `${cat.subtitle}｜${cat.label}` : cat.label}</Link>
                              : <h3 className="text-[16px] leading-[24px] font-black text-stone-900">{cat.subtitle ? `${cat.subtitle}｜${cat.label}` : cat.label}</h3>}
                          </div>
                          <div className="grid grid-cols-1 gap-1 pl-4">
                            {cat.children?.length ? cat.children.map(sub => (
                              <Link key={`nav-subchild-${sub.id}`} to={sub.url} target={sub.openInNewWindow ? '_blank' : undefined} className="block py-3 px-[16px] text-stone-600 hover:text-primary hover:bg-stone-50 rounded-lg transition-colors font-medium" onClick={() => { setIsMenuOpen(false); setActiveSubMenu(null); }}>{sub.label}</Link>
                            )) : cat.url && cat.url !== '#' && (
                              <Link to={cat.url} target={cat.openInNewWindow ? '_blank' : undefined} className="block py-3 px-[16px] text-stone-600 hover:text-primary hover:bg-stone-50 rounded-lg transition-colors font-medium" onClick={() => { setIsMenuOpen(false); setActiveSubMenu(null); }}>查看全部 {cat.label}</Link>
                            )}
                          </div>
                        </div>
                      ))}
