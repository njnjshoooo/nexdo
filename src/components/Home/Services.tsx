                ...
                <Card className={`group rounded-2xl border-none shadow-xl transition-colors duration-300 ${
                  ...
                  <div className={`flex flex-col lg:flex-row ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                    ...
                    <div className="lg:w-1/2 p-8 lg:p-16 flex flex-col justify-center">
                      <h3 className={`text-3xl font-bold mb-4 transition-colors duration-300 ${isPrimary ? 'text-white' : 'text-stone-900 group-hover:text-white'}`}>
                        {displayTitle} 
                      </h3>
                      
                      <p className={`text-lg mb-8 leading-relaxed transition-colors duration-300 ${
                        isPrimary ? 'text-white/90' : 'text-stone-600 group-hover:text-white/90'
                      }`}>
                        {displayDescription}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-3 mb-10">
                        {displayTags.map((tag: string, i: number) => (
                          <span 
                            key={i} 
                            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors duration-300 border ${
                              isPrimary 
                                ? 'bg-white/10 text-white border-white/20' 
                                : 'bg-stone-100 text-stone-500 border-transparent group-hover:bg-white/10 group-hover:text-white group-hover:border-white/20'
                            }`}
                          >
                            {tag}
                          </span>
                        ...
                        <Link 
                          ...
                          className={`inline-flex items-center gap-2 px-8 py-3 rounded-full text-sm font-bold transition-all shadow-md active:scale-95 ${
                            isPrimary
                              ...
                              : 'bg-[#885200] text-white hover:bg-[#704300] group-hover:bg-white group-hover:text-[#516438] hover:group-hover:bg-stone-100'
                          }`}
                        >
