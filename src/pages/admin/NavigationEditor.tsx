...
import { NavItem, NavItemTemplateType } from '../../types/navigation';
...
import { PageMainTitle, InputClass } from '../../components/admin/ui/AdminEditorUI';
...
function flattenForRender(items: NavItem[], expandedIds: Set<string>, parentId: string | null = null, depth = 0): FlattenedItem[] {
  return items.reduce<FlattenedItem[]>((acc, item) => {
    acc.push({ ...item, parentId, depth });
    if (expandedIds.has(item.id) && item.children && item.children.length > 0) {
      acc.push(...flattenForRender(item.children, expandedIds, item.id, depth + 1));
    }
    return acc;
  }, []);
}

function unflatten(flattenedItems: any[]): NavItem[] {
  if (!Array.isArray(flattenedItems)) return [];
  
  const root: NavItem[] = [];
  const map: Record<string, NavItem> = {};
  
  // 防禦：如果傳入的已經是處理好的樹狀結構，直接回傳
  if (flattenedItems.length > 0 && 'children' in flattenedItems[0] && !('parent_id' in flattenedItems[0]) && !('parentId' in flattenedItems[0])) {
  ...
  flattenedItems.forEach(item => {
    ...
    map[item.id] = { 
      ...
      label: item.label,
      url: item.url,
      templateType: item.template_type || item.templateType,
      pageSlug: item.page_slug || item.pageSlug,
      openInNewWindow: item.open_in_new_window ?? item.openInNewWindow ?? false,
      children: [] 
    };
  });
    ...
    <div ref={setNodeRef} style={style} className="mb-2">
      <div className="bg-white border border-stone-200 rounded-xl p-3 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 grid grid-cols-12 gap-3 items-center">
          <input type="text" value={item.label} onChange={(e) => onUpdate(item.id, { label: e.target.value, isCustomLabel: true })} className={`col-span-4 ${InputClass} !py-1.5 !px-3 font-bold`} />
          <input type="text" value={item.url} onChange={(e) => onUpdate(item.id, { url: e.target.value })} className={`col-span-6 ${InputClass} !py-1.5 !px-3 font-mono`} />
...
export default function NavigationEditor() {
  ...
  // 💡 修正後台載入邏輯：與前台事件同步偵聽，徹底解決非同步時間差問題
  useEffect(() => {
    const updateEditorData = () => {
      ...
      if (settings && settings.items) {
        ...
        // 預設將所有有小孩的節點展開
        const allIdsWithChildren = flatten(settings.items, new Set())
          .filter(i => i.children && i.children.length > 0)
          .map(i => i.id);
        setExpandedIds(new Set(allIdsWithChildren));
      }
    };
    ...
    // 2. 觸發一次強制刷新撈取 Supabase
    navigationService.refresh().then(() => {
      updateEditorData();
    });

    // 3. 註冊全域事件監聽，一旦前台或 Service 更新完成，立刻強行更新後台畫面
    const handleRefresh = () => {
      updateEditorData();
    };
    window.addEventListener('navigation_refreshed', handleRefresh);

    setPages(pageService.getAll());
    setArticles(articleService.getAll());

    return () => {
      window.removeEventListener('navigation_refreshed', handleRefresh);
    };
  }, []);

  const flattenedItems = useMemo(() => flattenForRender(items, expandedIds), [items, expandedIds]);
  const usedPageIds = useMemo(() => {
    const used = new Set<string>();
    flatten(items, new Set()).forEach(i => {
      if (i.pageSlug) used.add(i.pageSlug);
      // 容錯：如果有舊資料沒有 pageSlug，從網址推斷
      if (i.url) {
        if (i.url.startsWith('/blog/')) used.add(i.url.replace('/blog/', ''));
        else if (i.url.startsWith('/')) used.add(i.url.substring(1));
      }
    });
    return used;
  }, [items]);
  ...
  const handleAddItem = (item: any, type: 'page' | 'article' | 'custom') => {
    let url = '#';
    let templateType: NavItemTemplateType = 'GENERAL';
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeIndex !== -1 && overIndex !== -1) {
      ...
      const dragDepth = Math.round(deltaX / 32);
  ...
  const handleSave = async () => {
    ...
    try {
      const allFlattened = flatten(items, new Set());
      
      const dbItems = allFlattened.map((item, index) => ({
        id: item.id,
        label: item.label,
        url: item.url,
        template_type: item.templateType,
        page_slug: item.pageSlug || null,
        open_in_new_window: item.openInNewWindow || false,
        parent_id: item.parentId || null,
        sort_order: index + 1
      }));

      await navigationService.updateSettings(dbItems);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <PageMainTitle className="!text-3xl mb-2">導覽列管理</PageMainTitle>
          <p className="text-stone-500">管理網站選單結構與層級</p>
        </div>
        <SaveButton onClick={handleSave} status={saveStatus} label="儲存變更" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="w-full lg:w-80 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
            <button onClick={() => setExpandedSection(expandedSection === 'pages' ? null : 'pages')} className="w-full p-4 flex items-center justify-between bg-stone-50 hover:bg-stone-100 transition-colors">
            ...
            {expandedSection === 'pages' && (
              <div className="p-4 border-t border-stone-200">
                <AdminSearchInput placeholder="搜尋頁面名稱..." value={searchQueryPages} onChange={(e) => setSearchQueryPages(e.target.value)} className="mb-4" />
                <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
                  {pages.filter(p => p.isPublished && p.title.includes(searchQueryPages) && !(usedPageIds.has(p.slug) || usedPageIds.has(p.id))).map(p => (
                    <SidebarItem key={p.id || p.slug} item={p} onAdd={handleAddItem} type="page" />
                  ))}
                  {pages.filter(p => p.isPublished && p.title.includes(searchQueryPages) && !(usedPageIds.has(p.slug) || usedPageIds.has(p.id))).length === 0 && (
          ...
          {/* Articles Accordion */}
          <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
            <button onClick={() => setExpandedSection(expandedSection === 'articles' ? null : 'articles')} className="w-full p-4 flex items-center justify-between bg-stone-50 hover:bg-stone-100 transition-colors">
            ...
            {expandedSection === 'articles' && (
              <div className="p-4 border-t border-stone-200">
                <AdminSearchInput placeholder="搜尋文章名稱..." value={searchQueryArticles} onChange={(e) => setSearchQueryArticles(e.target.value)} className="mb-4" />
                <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
                  {articles.filter(a => a.isPublished && a.title.includes(searchQueryArticles) && !(usedPageIds.has(a.slug) || usedPageIds.has(a.id))).map(a => (
                    <SidebarItem key={a.id || a.slug} item={a} onAdd={handleAddItem} type="article" />
                  ))}
                  {articles.filter(a => a.isPublished && a.title.includes(searchQueryArticles) && !(usedPageIds.has(a.slug) || usedPageIds.has(a.id))).length === 0 && (
          ...
          <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
            <button onClick={() => setExpandedSection(expandedSection === 'custom' ? null : 'custom')} className="w-full p-4 flex items-center justify-between bg-stone-50 hover:bg-stone-100 transition-colors">
              <span className="font-bold text-stone-700">自訂連結</span>
              {expandedSection === 'custom' ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
            </button>
            {expandedSection === 'custom' && (
              <div className="p-4 border-t border-stone-200">
                <input type="text" placeholder="連結文字" className={`mb-3 ${InputClass}`} value={customTitle} onChange={(e) => setCustomTitle(e.target.value)} />
                <input type="text" placeholder="網址 (例如: https://...)" className={`mb-4 ${InputClass}`} value={customUrl} onChange={(e) => setCustomUrl(e.target.value)} />
                <button 
                  onClick={() => {
                  ...
                  className="w-full py-2 bg-stone-800 text-white rounded-xl text-sm font-medium hover:bg-stone-900 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                >
                  <Plus size={16} /> 新增至導覽列
                </button>
              </div>
