...
import { Package, ArrowRight, ExternalLink, RefreshCw, Edit2, X, Plus, Trash2, FileText, ArrowLeft, CheckCircle } from 'lucide-react';
...
export default function ProductEditor() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      if (isEditing && id) {
        ...
        if (prod) {
          setEditedProduct(JSON.parse(JSON.stringify(prod)));
          setOriginalId(prod.id);
          setTempOrderCode(prod.orderCode || '');
        } else {
        ...
        const newProduct: Product = {
          ...
          requireTime: false,
          requireNotes: false,
          fixedConfig: { price: 0, unit: '', buttonText: '預約', enableDeposit: false },
          checklist: [],
          images: [],
          variants: [],
    ...
    try {
      if (newId !== originalId) {
        ...
        if (isEditing) {
          ...
          await productService.create(editedProduct);
        }
      }
    } catch (err) {
  ...
  if (!editedProduct) return <div className="p-8">載入中...</div>;

  return (
    <div className="max-w-7xl mx-auto pb-20 relative">
      {toast && (
        <div className="fixed bottom-6 right-6 bg-stone-800 text-white px-6 py-3 rounded-xl shadow-xl z-50 flex items-center gap-2 animate-in fade-in slide-in-from-bottom-4">
      ...
      <AdminPageHeader
        title={isEditing ? '編輯產品核心資訊' : '新增產品核心資訊'}
        backTo="/admin/products"
        actionButtons={<SaveButton status={saveStatus} onClick={handleSave} type="button" />}
      />

      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 mt-6 overflow-hidden">
        {/* Header inside form */}
        ...
        {/* Tabs - UI like FinanceManagement */}
                ...
                {editedProduct.orderMode === 'FIXED' ? (
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <input 
                        type="number" 
                        value={editedProduct.fixedConfig?.price || 0}
                ...
                <div className="p-6 bg-stone-50 rounded-2xl border border-stone-200 space-y-4">
                  ...
                  <div className="space-y-4">
                    {editedProduct.variants && editedProduct.variants.map((variant, index) => (
                      <div key={variant.id} className="p-4 bg-white border border-stone-200 rounded-xl relative group">
                        <button
                          onClick={() => {
                            const vars = [...editedProduct.variants!];
                            vars.splice(index, 1);
                            updateProductField('variants', vars);
                          }}
                          className="absolute -right-2 -top-2 w-6 h-6 bg-red-100 text-red-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-200"
                        >
                          <X size={14} />
                        </button>
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex gap-2">
                            <div className="flex-1">
                              <label className="block text-[10px] font-bold text-stone-400 uppercase mb-1">價格</label>
                              <input
                                type="number"
                                value={variant.price}
                                onChange={(e) => {
                                  const vars = [...editedProduct.variants!];
                                  vars[index].price = parseInt(e.target.value) || 0;
                                  updateProductField('variants', vars);
                                }}
                                className="w-full px-3 py-1.5 border rounded-lg text-sm"
                              />
                            ...
                            <div className="w-20">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const vars = [...editedProduct.variants!];
                                  vars[index].unit = e.target.value;
                                  updateProductField('variants', vars);
                                }}
                                className="w-full px-3 py-1.5 border rounded-lg text-sm"
                                placeholder="/個"
                    ...
                    {(!editedProduct.variants || editedProduct.variants.length === 0) && (
                      <div className="text-center py-6 text-sm text-stone-400 border-2 border-dashed rounded-xl">
                        此產品目前沒有多種規格。如需讓客戶選擇，點擊上方新增。
                      </div>
                    )}
                  </div>
                </div>
