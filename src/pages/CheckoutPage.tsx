...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result?.error || '建立訂單失敗');

      // 已登入者順手更新個人資料（不阻擋付款流程）
      if (user) {
        updateProfile({
          name: formData.name,
          phone: formData.phone,
      ...
      // ⚠️ 跳轉到綠界必須用 form POST，不能用 fetch 或 window.location
      const form = document.createElement('form');
      form.method = 'POST';
      form.action = result.apiUrl;
      Object.entries(result.params as Record<string, string>).forEach(([key, value]) => {
      ...
      setError(err?.message || '建立訂單時發生錯誤，請稍後再試');
      setLoading(false);
    }
  };
              ...
              <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    { value: 'ALL', label: '到下一步再選擇' },
                  ].map(opt => (
                    <label
                      key={opt.value}
                      className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${formData.paymentMethod === opt.value ? 'border-primary bg-primary/5' : 'border-stone-100 hover:border-stone-200'}`}
                    >
                      <input
                        type="radio"
                        name="paymentMethod"
                        ...
                        className="hidden"
                      />
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${formData.paymentMethod === opt.value ? 'border-primary' : 'border-stone-300'}`}>
                        {formData.paymentMethod === opt.value && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
                      </div>
                ...
                <p className="mt-4 text-xs text-stone-400">
                  點擊「前往付款」後將跳轉至綠界 ECPay 安全付款頁完成交易
                </p>
              </section>

              {error && (
                <div className="bg-red-50 border-2 border-red-100 rounded-2xl p-4 text-sm text-red-700">
                  {error}
                </div>
              )}
