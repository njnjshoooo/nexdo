...
/** 適合誰：先讓訪客對號入座，再往下看服務內容。 */
const SITUATIONS: [string, string][] = [
  ['想把家住得更輕鬆', '走道被物品佔住、常用的東西找不到，希望日常動線順一點，也降低跌倒的機會。'],
  ['想先安排重要的物品', '證件、保單、印章與紀念物散在各處，想趁自己還做得動時，先整理成家人找得到的樣子。'],
  ['正在面對生活轉換', '退休、換屋、與子女同住，或家中剛有成員離開，需要有人陪著一起決定物品的去留。'],
  ['家人住得遠，想幫忙安排', '子女不在身邊，想先了解服務方式與費用，再與長輩討論要不要進行。'],
];
...
const PACES: { label: string; title: string; text: string; items: string[] }[] = [
  {
    label: 'WITH YOU',
    title: '陪伴型',
...
export default function OrganizingPage() {
  ...
  return <div className="organizing-product">
    ...
    <section className="retirement-section"><div className="retirement-container">
      <div className="section-heading"><p className="eyebrow">適合誰</p><h2>不用先整理好，<br />我們先聽您說。</h2><p>不必一次整理整個家，也不用先說服自己放下。從一個抽屜、一疊照片，或一件一直掛心的事開始都可以。</p></div>
      <div className="organizing-cards">{SITUATIONS.map(([title, text]) => <article className="organizing-card" key={title}><h3>{title}</h3><p>{text}</p></article>)}</div>
    </div></section>
    ...
    <section className="retirement-section organizing-section"><div className="retirement-container">
      ...
      <div className="organizing-steps">{STEPS.map((step, i) => <article key={step.title}><span className="service-number">0{i + 1}</span><h3>{step.title}</h3><p>{step.text}</p><ul className="organizing-list">{step.items.map(item => <li key={item}>{item}</li>)}</ul></article>)}</div>
    </div></section>

    <section className="retirement-section"><div className="retirement-container">
      <div className="section-heading"><p className="eyebrow">整理方式</p><h2>想慢慢選，或快一點完成，<br />都有合適的做法。</h2></div>
      <div className="organizing-pace-grid">{PACES.map(pace => <article className="organizing-card" key={pace.title}><p className="eyebrow">{pace.label}</p><h3>{pace.title}</h3><p>{pace.text}</p><ul className="organizing-list">{pace.items.map(item => <li key={item}>{item}</li>)}</ul></article>)}</div>
